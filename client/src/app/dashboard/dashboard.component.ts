import { Component, OnInit, Input, Inject, HostListener, ViewChild, ComponentFactoryResolver, OnChanges, ChangeDetectionStrategy, SimpleChanges, ViewContainerRef,} from '@angular/core';
...
export class DashboardComponent implements OnInit, OnChanges {
  @ViewChild('currencycontainer', { read: ViewContainerRef }) entry: ViewContainerRef;
  ...
  constructor(private dataService: DataService,
              private title: Title,@Inject(DOCUMENT) document: any,
  ...
  ngOnInit() {
    ...
    this.dataService.currencies$.subscribe((data)=> { this.currencies = data})
    ...
    this.route.params.subscribe((params: Params)=> {
      ...
      this.articles$.pipe(       
        map((data:Article[])=>{
          return data.filter((article:Article) => article.title === this.article_title);
        })  
        ,
        take(1),             
  ...
  ngAfterViewInit(): void {
    this.loading = this.loadingService.loading$.pipe().subscribe(
      (status: boolean) => {
  ...
  searchChange(term: string){
    // console.log("aaaa", term);
      ...
      if(term === "News"){
     of(this.tmp_articles).pipe(       
        map((data:Article[])=>{
          return data.filter((article:Article) => article.source.type === "News");
        })  
        ,
        ...
        of(this.tmp_articles).pipe(       
          map((data:Article[])=> data.filter((article:Article) => article.source.type === "Media"))  
          ,
          filter((articless:Article[])=>{           
            return articless && articless.length > 0
         })        
      ...
      of(this.tmp_articles).pipe(
        map((data:Article[])=>{
         return data.filter((article:Article) =>
          {
            if(article.title.toLocaleLowerCase().includes(term.toLocaleLowerCase()))
            return true;
  ...
  createCurrencyComponent(currency:Currency) {
    ...
    if( this.entry !== undefined){
      this.entry.clear();
      const factory = this.resolver.resolveComponentFactory(CurrencyDetailsComponent);
