...
class Source < ApplicationRecord
  ...
  def source_fetch
    p self
    Source::FetchWorker.perform_async(self.id)
  end
  ...
  def self.get_feed(source)
    ...
    @articles.each do |article|
      ...
      if found_article.length == 0 || found_article.nil?
        ...
        new_article = Article.new(:title => article["title"],
                                  ...
                                  :source_id => source.id)
        puts "new:::::::::", new_article
        if new_article.valid?
        puts "feed:",source.title, @articles,   new_article.save!
