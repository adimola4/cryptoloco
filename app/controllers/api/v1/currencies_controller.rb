...
class Api::V1::CurrenciesController < ActionController::API
  ...
  def index
    ...
    @currencies.each do |c|
      @res << {
        ...
        'v24': formatting_currency(c.v24),
        'p24': formatting_currency(c.p24),
        'p1h': formatting_currency(c.p1h),
        'p7d': formatting_currency(c.p7d),
        'links': c.links,
        'img_url': c.img_url
      }
    end
  ...
  def show
    ...
    @res = {
      ...
      'price_usd': formatting_currency(@currency[0].price_usd),
      'v24': formatting_currency(@currency[0].v24),
      'p24': formatting_currency(@currency[0].p24),
      'p1h': formatting_currency(@currency[0].p1h),
      'p7d': formatting_currency(@currency[0].p7d),
  ...
  def formatting_currency(number)
    "%g" % ("%.6f" % number)
  end
end
